import React from "react";
import { Instagram, Facebook, Youtube } from "lucide-react";

const socialBtn =
  "w-9 h-9 rounded-full bg-surface/10 border border-surface/10 hover:border-mango/40 hover:bg-mango hover:text-on-surface flex items-center justify-center text-surface-variant transition-all hover:scale-110";

function SpotifyIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z" />
    </svg>
  );
}

function WhatsAppIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.435 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  );
}

export default function Footer() {
  return (
    <footer id="contacto" className="py-12 px-6 md:px-16 bg-on-surface border-t border-surface/10 relative z-10 scroll-mt-20">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          
          {/* Left: Copyright & Credit metadata */}
          <div className="text-center md:text-left order-3 md:order-1 md:w-1/3">
            <p className="text-surface-variant/70 font-anybody font-bold text-[10px] uppercase tracking-[0.2em]">
              © 2026 SON HAVANA CLUB
            </p>
            <p className="text-[9px] text-surface-variant/70 uppercase tracking-widest mt-1">
              Todos los derechos reservados &bull; Rumba, clave y tradición
            </p>
          </div>

          {/* Middle: Grayscale/White tinted brand Logo */}
          <div className="flex justify-center order-1 md:order-2 md:w-1/3">
            <a href="#hero" className="inline-block transition-transform hover:scale-105">
              <img
                referrerPolicy="no-referrer"
                alt="Son Havana Club Logo"
                className="h-12 w-auto grayscale brightness-200 opacity-65 hover:opacity-100 transition-all"
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuCyQX7vQQb-Iq8CK8GkvbiGjMfQCuOSHxtdqT6R69pDe2Ty7YNgBVc--mJJ2_myP_h-bHdTtYftyky-VSt7off2WKbWjXj__dvaa-wNJ-l5LWi1K4xYDggXOln38uTQzMSad_zN5-OEpM43jyoBL1ZminsBgxftLNkigrfgXGUTCJYC-Vqifw7rhzU-O5wlUMF4s1mJho8tpmkhOZwFVAKDEgc4VNbN0yIw_BQAcOGkRg-v4v91EhJgXdhZ0MoHc2ZFvQ"
              />
            </a>
          </div>

          {/* Right: redes + WhatsApp */}
          <div className="flex gap-4 justify-center md:justify-end items-center order-2 md:order-3 md:w-1/3">
            <span className="text-[10px] font-anybody font-black text-mango/85 tracking-widest uppercase hidden lg:inline">
              SÍGUENOS EN:
            </span>
            <div className="flex gap-3">
              <a
                href="https://www.instagram.com/sonhavana"
                target="_blank"
                rel="noopener noreferrer"
                className={socialBtn}
                aria-label="Instagram Son Havana"
                title="Síguenos en Instagram"
              >
                <Instagram className="w-4 h-4" />
              </a>
              <a
                href="https://www.facebook.com/sonhavana/"
                target="_blank"
                rel="noopener noreferrer"
                className={socialBtn}
                aria-label="Facebook Son Havana"
                title="Síguenos en Facebook"
              >
                <Facebook className="w-4 h-4" />
              </a>
              <a
                href="https://www.youtube.com/@juliosonhavana"
                target="_blank"
                rel="noopener noreferrer"
                className={socialBtn}
                aria-label="YouTube Julio Son Havana"
                title="Canal de YouTube"
              >
                <Youtube className="w-4 h-4" />
              </a>
              <a
                href="https://open.spotify.com/user/31jw5jzeoqa2f5kttbqt55ei3kqi?si=4f3a46ca591a435f"
                target="_blank"
                rel="noopener noreferrer"
                className={socialBtn}
                aria-label="Spotify Julio Restrepo Molina"
                title="Escúchanos en Spotify"
              >
                <SpotifyIcon className="w-4 h-4" />
              </a>
              <a
                href="https://wa.me/573105156550"
                target="_blank"
                rel="noopener noreferrer"
                className={socialBtn}
                aria-label="WhatsApp Son Havana"
                title="WhatsApp Directo"
              >
                <WhatsAppIcon className="w-4 h-4" />
              </a>
            </div>
          </div>

        </div>
      </div>
    </footer>
  );
}
